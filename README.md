# AETHER

Original fictional AI company website, built with Vite + React + Three.js + GLSL + GSAP + Framer Motion.

## Run

npm install
npm run dev

## Build

npm run build
npm run preview

## Experience

- Interactive GLSL particle sphere responding to the cursor.
- Scroll morphs: sphere, neural network, dispersed particles, the core statement, and an AETHER mark.
- Product concepts: Canvas, Studio, Relay.
- Model concepts: Spark, Atlas, Prism.
- JavaScript/Python illustrative API tabs and clipboard copying.
- Monthly/yearly illustrative pricing and accessible explanation dialogs.
- Responsive navigation and reduced-motion support.

AETHER, its products, models, API examples and pricing are fictional design concepts. There are no real customer endorsements, performance benchmarks, live AI requests, authentication, payments or subscriptions. The API examples must not be used as documentation for an existing SDK.

Google Fonts supplies DM Sans and Manrope. All primary visuals are generated in the browser with Three.js and shaders. No background video is used.
